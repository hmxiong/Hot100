...
def run_reference_batch(
    ...
    for i in range(outputs.shape[0]):
        ...
        normalized.append({"token_ids": [int(x) for x in response_ids], "text": tokenizer.decode(response_ids)})
    return normalized
...
def main():
    parser = argparse.ArgumentParser(description="验证空位补请求后的 refillable shrinking-batch 行为")
    ...
    parser.add_argument(
        ...
        default="4,8,16,32,6,10",
        type=str,
        help="为每条 prompt 单独指定 max_new_tokens，使用逗号分隔",
    )
    parser.add_argument("--prompts", default="", type=str, help="自定义 prompt，使用 || 分隔")
    parser.add_argument("--num_prompts", default=6, type=int)
    ...
    engine, tokenizer = init_engine(args.load_from, args.device, int(args.max_batch_size))
    params = build_params(
        tokenizer=tokenizer,
    ...
    for idx, (prompt, sp) in enumerate(zip(prompts, params)):
        ...
        seq_id_to_prompt_idx[int(rid)] = idx
    ...
    print("=== Refillable Shrinking Batch Trace ===")
    print(
    ...
    try:
        while not engine.is_finished():
            ...
            if len(running_after) < len(running_before):
                shrink_happened = True
            if waiting_after != waiting_before and len(running_after) >= len(running_before):
            ...
            admitted_now = [seq_id for seq_id in running_after if seq_id not in running_before]
            if admitted_now:
                print(f"  admitted_now={admitted_now}")
        ...
        print("[FAIL] refillable shrinking-batch trace 运行失败")
    ...
    for prompt_idx in range(len(prompts)):
        ...
        ordered_outputs.append({"token_ids": token_ids, "text": tokenizer.decode(token_ids)})

    print("=== Refillable Summary ===")
    print(f"total_steps={step_idx}")
    print(f"shrink_happened={shrink_happened}")
    print(f"refill_happened={refill_happened}")
    print()
    ...
    ref_engine, ref_tokenizer = init_engine(args.load_from, args.device, int(args.max_batch_size))
    ...
    print()
    if ok:
        print("[PASS] refillable shrinking-batch 路径最终输出与 reference engine.generate_batch 完全一致")
    else:
        print("[FAIL] refillable shrinking-batch 路径最终输出与 reference engine.generate_batch 不一致")
