...
- Q：多层 sigmoid 网络为什么会梯度消失？图里 A/B/C/D 对应哪一层（离输出近还是离输入近）？
   A：sigmoid 在饱和区导数很小（最大也只有 0.25），反向传播的梯度会按链式法则连乘这些导数，导致越靠近输入的层梯度越小、学习越慢。
  
  - 对应关系：A（梯度最大/学习最快）→ 最靠近输出的隐藏层；B → 次靠近输出；C → 更靠近输入；D（梯度最小/学习最慢）→ 最靠近输入的隐藏层。
  - 原因口径：对第 l 层的梯度大致包含 ∏_{k>l} σ'(z_k)，而 σ'(z)≤0.25，深度一增加梯度就指数级衰减；再叠加激活饱和（|z|大）使 σ'(z)≈0，会出现前面层几乎不更新。
- Q：DP/TP/PP 分别解决什么瓶颈？
   A：
